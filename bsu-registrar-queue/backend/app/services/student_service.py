...
class StudentService:
    ...
    def delete_student(self, student_id: int) -> bool:
        ...
        Raises ValueError if the student has any tickets or appointments
        (even completed/cancelled/old ones) - deleting them would violate
        the tickets.student_id / appointments.student_id foreign keys and
        silently destroy historical records, so we block it with a clear
        message instead of letting the database raise a raw IntegrityError
        (which the API's catch-all handler would otherwise turn into an
        opaque 500).
        """
        from ..db_models import AppointmentDB, TicketDB
        ...
        appointment_count = self.db.query(func.count(AppointmentDB.id)).filter(
            AppointmentDB.student_id == student_id
        ).scalar()
        if appointment_count:
            raise ValueError(
                f"Cannot delete student '{student.student_id}': they have {appointment_count} "
                f"appointment(s) on record."
            )
